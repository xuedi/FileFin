...
flowchart TD
    ...
    MAP --> M["match entries to library<br/>(shared year-strict matcher)"]
    M --> R["review table:<br/>exact rows checked, approx rows unchecked"]
...
- The fetch pages through the API's `paging.next` links until exhausted. Each node maps to one
  entry: the romaji `title` is the primary, the English `alternative_titles.en` becomes an alias
  the matcher can fall back to.
...
Matching is the shared, year-strict matcher described in [`mdl.md`](mdl.md#matching): titles are
normalized to lowercase alphanumerics, an exact **year** match is flagged exact and pre-selected,
and a year-mismatched candidate is offered but left **unselected** (tagged `approx: year x != y`).
For MyAnimeList the entry also carries its English alias, so a romaji title can still match a
library item filed under its English name.
