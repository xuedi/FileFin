...
- **Refill (actionable).** "Needs attention" is exactly what the scan buttons do: enqueue an
  optimize / enrich / thumbnail / probe / TMDb / people task, and the relevant agent handles it. The
  ...
  `enricher.md`, `thumbnailer.md`, `probe.md`, `tmdb.md`, `people.md`).
...
flowchart TD
    ...
    ADD --> REFILL[refill optimize / enrich / thumbnail / probe / TMDb / people queues]
    DEL --> REFILL
